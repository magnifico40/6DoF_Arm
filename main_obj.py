#6DOF ARM model build from .obj files
...
from RobotArm_numerical import RobotArm
import pywavefront
...
class RobotOpenGLWidget(QOpenGLWidget):
    def __init__(self, robot, parent=None):
        ...
        self.quadric = None
        self.segment_models = [
            pywavefront.Wavefront('Models3D/J1.obj', collect_faces=True),
            pywavefront.Wavefront('Models3D/J2.obj', collect_faces=True),
            pywavefront.Wavefront('Models3D/J3.obj', collect_faces=True),
            pywavefront.Wavefront('Models3D/J4.obj', collect_faces=True),
            pywavefront.Wavefront('Models3D/J5.obj', collect_faces=True),
            pywavefront.Wavefront('Models3D/J6.obj', collect_faces=True)
        ]
    ...
    def draw_grid(self):
        ...
        glBegin(GL_LINES)
        glVertex3f(0, -grid_size * spacing, 0.01)
        glVertex3f(0, grid_size * spacing, 0.01)
        glEnd()
        # Y arrow
        glBegin(GL_LINES)
        glVertex3f(0, arrow_pos, 0.01)
        glVertex3f(arrow_size, arrow_pos - arrow_size, 0.01)
        glVertex3f(0, arrow_pos, 0.01)
        glVertex3f(-arrow_size, arrow_pos - arrow_size, 0.01)
        glEnd()
        ...
        glBegin(GL_LINES)
        glVertex3f(-grid_size * spacing, 0, 0.01)
        glVertex3f(grid_size * spacing, 0, 0.01)
        glEnd()
        # X arrow
        glBegin(GL_LINES)
        glVertex3f(arrow_pos, 0, 0)
        glVertex3f(arrow_pos - arrow_size, arrow_size, 0.01)
        glVertex3f(arrow_pos, 0, 0)
        glVertex3f(arrow_pos - arrow_size, -arrow_size, 0.01)
        glEnd()
    ...
    def apply_inverse_kinematics(self):
        ...
        for i in range(6):
            try:
                ...
                print("cannot achieve desired position")

...
from RobotArm_numerical_path import RobotArm
...
class MainWindow(QMainWindow):
    ...
    def reset_all_joints(self):
        for slider in self.sliders:
            ...
            animation.setDuration(500)  #animation time
    ...
    def PickTarget(self):
        ...
        if self.PickTarget_btn.isChecked():
            ...
            targetToPick_x1 = self.x_spinbox_targ.value()   
            targetToPick_x = -self.y_spinbox_targ.value()
            targetToPick_y1 = self.y_spinbox_targ.value()
            targetToPick_y = self.x_spinbox_targ.value()
            targetToPick_z = self.z_spinbox_targ.value()
            targetToPickXYZ = [targetToPick_x, targetToPick_y, targetToPick_z]
            targetToPickXYZ1 = [targetToPick_x1, targetToPick_y1, targetToPick_z]

            initial_angles = self.robot.get_angles()
            yaw = np.degrees(np.arctan2(targetToPick_y1, targetToPick_x1))

            test_angles = [0, 45, -30, 0, 90, 0]
            result1 = self.robot.kinematics(test_angles)
            x1, y1, z1, yaw1, pitch1, roll1 = self.robot.get_gripper_xyz_ypr()

            ypr = [yaw, 90, 180] 
            Targetangles, error = self.robot.inverse_kinematics(targetToPickXYZ1, ypr)
    ...
    def draw_targetToPick(self):
        if self.draw_target_btn.isChecked():
            targetToPick_x = -self.y_spinbox_targ.value()
            targetToPick_y = self.x_spinbox_targ.value()
            targetToPick_z = self.z_spinbox_targ.value() / 2
            targetToPickXYZ = [targetToPick_x, targetToPick_y, targetToPick_z]

            
            self.robot.set_targetToPick_xyz(targetToPickXYZ)
    ...
    def move_pointToPoint(self, endingAngles, initialAngles):
        angles_deg_int = np.rint(endingAngles).astype(int)
        trajectory = self.robot.joint_trajectory(initialAngles, endingAngles)
        for q in trajectory:
    ...
    def apply_inverse_kinematics(self):
        ...
        x, y, z, yaw, pitch, roll = self.robot.get_gripper_xyz_ypr()
        #get results
        angles, error = self.robot.inverse_kinematics([target_x, target_y, target_z], [target_yaw, target_pitch, target_roll])        
